-- Sistema de Landing Page - Tabelas para Leads e Pagamentos (CORRIGIDO)
...
CREATE TABLE IF NOT EXISTS landing_payments (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'pending', -- 'pending', 'received', 'confirmed', 'overdue', 'cancelled'
...
-- Trigger para executar a função após pagamento confirmado
...
COMMENT ON FUNCTION create_admin_after_payment() IS 'Cria admin e campanha automaticamente após pagamento confirmado';
